...
eval_f_logs_weighted <- function(x, r45, prevs, incidence, w){
  ...
  # sums of squares for prevalences
  sumsquare.prev <- w1 * sum((log(prev.inc[, 1:8] + .0001) - log(prevs + .0001)) ^ 2)
  # sums of squares for log(incidence), only ages 65:90
  sumsquare.inc <- w2 * (n1 / n2) * sum((log(prev.inc[16:41, 9] * 100) - log(incidence)) ^ 2)
...
make_prevplot_data <- function(sol){
  mats <- make_trans_matrix(sol, r45.params)
  
  prevs.c <- Prevrate.f.multi.ATN(ages, k0 = mats[[1]], k1 = mats[[2]])
  prevs.u <- Prevrate.f.multi.ATN.uncond(ages, k0 = mats[[1]], k1 = mats[[2]])
  ...
  dat.prev <- cbind.data.frame(c(as.vector(prevs.u), as.vector(fem_prev_u)),
                               ...
                               rep(c("Fitted", "Jack"), each = length(ages) * 8))
...
make_incplot_data <- function(sol){
  mats <- make_trans_matrix(sol, r45.params)
