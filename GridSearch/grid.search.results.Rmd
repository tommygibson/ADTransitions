...
incplot <- inc.consolidated %>%
  ...
  guides(alpha = FALSE) +
  labs(title = "Figure 3: Optimized and Empirical Target") +
  theme_bw()
...
We ran each optimization to convergence. The minimum optimal value of the objective function was `r min(optvals)`, and the largest optimal value was `r max(optvals)`. Each optimization converged within `r max(iters)` iterations.
...
The figures below show the transition probabilities for every transition $r_{ij}(a)$ for ages $a \in (50, \dots, 95)$. Figure 1 has all y-axes fixed from 0 to 1, while Figure 1 has scales unique to each panel. The red line in each panel corresponds to transition probabilities for the optimal parameters in Table \@ref(tab:optparams).
