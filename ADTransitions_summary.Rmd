...
Here we use $\boldsymbol{k}$ to represent the vector of 24 parameters $(\log(k_0), k_1)_{(ij)}$ for preclinical transitions $(r_{12}, \dots, r_{94})$. Optimization is with respect to the parameters $\boldsymbol{k}$. We use the COBYLA optimization algorithm in the R package `nloptr`, which accepts both linear and nonlinear constraints. In addition to the constraints \@ref(eq:constraints) we impose constraints such that the sum of transitions out of state $i$ for age $a$ is less than 1 up to age 95
\begin{align}
...
kable(lifetime.table.f, format = "latex", caption = "Lifetime risk of AD for women by age and disease state") %>%
...
kable(lifetime.table.m, format = "latex", caption = "Lifetime risk of AD for men by age and disease state") %>%
  kable_styling(latex_options = "HOLD_position")
...
```{r condf, fig.height = 7}

avg.cond.plot.f

```

```{r condm, fig.height = 7}
